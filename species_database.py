    ...
    for shape in ['square', 'triangle', 'hexagon']:
        ...
        for birth_rules in birth_rule_combinations:
            for survival_rules in survival_rule_combinations:
                ...
                species_name = f"{shape}:{birth_bits}{survival_bits}"
...
def create_species_database():
    ...
    # Add shape column if it was missing in an existing DB
    c.execute("PRAGMA table_info(species)")
    existing_columns = [row[1] for row in c.fetchall()]
    if 'shape' not in existing_columns:
        c.execute('ALTER TABLE species ADD COLUMN shape TEXT')
    ...
    c.executemany('''
        INSERT OR IGNORE INTO species (name, birth_rules, survival_rules, shape)
        VALUES (?, ?, ?, ?)
    ''', species_list)
